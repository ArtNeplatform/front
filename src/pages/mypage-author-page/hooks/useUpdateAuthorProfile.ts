...
import { updateAuthorProfile } from '@/apis/mypageAuthor/profile';
import { TUpdateAuthorProfile } from '@/apis/mypageAuthor/type';
...
export const useUpdateAuthorProfile = () => {
  ...
  const {
    ...
    onError: (error) => {
      toast.error(`작가 프로필 수정에 실패했습니다. ${error.message}`);
    },
  });
